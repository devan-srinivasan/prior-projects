...
class GenreGraph:
    ...
    * There will be a larger description when we add more to this class *
...
def load_genres(genres_file: str) -> dict[str, dict[str, float]]:
    """
    load and return all genres from the given genres file into a dict that maps genre to its
    average properties
...
def load_artists_to_genres(art_genres_file: str) -> dict[str, list[str]]:
    """
    return a mapping of every artist within the given artist with genres file to the
...
def clean_lists(lst: str) -> list[str]:
    """
    Clean the artists from the csv file.

    NOTE THAT THIS STILL NEEDS TO BE EDITED A LITTLE

    artists column is inconsistent there are extra " and ' at the beginning and end and there are
    also random slashes -> \\ too (slashes come before ' from what I see). Also remove the
...
def create_song_graph(songs: list[Song], threshold: float) -> SongGraph:
    """
    Take a list of songs and return their songGraph. The song graph connects songs which have
...
def create_genre_graph(songs_file: str, artists_file: str, genres_file: str,
    ...
    """
    Return the main genre graph to be used to recommend songs.
